    ...
    <Table celled selectable textAlign={'center'}>
      ...
      <Table.Body>
        {calendars.map((calendar) => {
          ...
          const duration = moment(calendar.end_date).diff(moment(calendar.start_date), 'days');
          const dDay = moment(calendar.start_date).diff(moment(), 'days');

          return (
            <Link href={`/board/calendar/update/${calendar.id}`} key={calendar.id}>
              <Table.Row key={calendar.id} positive={isActive}>
                ...
                <Table.Cell>
                  ...
                  {moment(calendar.end_date).format('YYYY-MM-DD')}{' '}
                  ({duration}일)
                </Table.Cell>
                <Table.Cell>
                  {dDay ? `D-${dDay}` : 'D-Day'}
